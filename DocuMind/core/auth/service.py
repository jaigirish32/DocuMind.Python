...
async def register_user(
    ...
    try:
        ...
        logger.info("User registered", username=username, user_id=user_id, company=company)
...
async def login_user(
    ...
    """
    cursor = await db.execute(
        "SELECT id, username, email, company, password_hash FROM users WHERE username = ?",
        (username.lower().strip(),),
    )
...
async def get_user_by_id(
    ...
    cursor = await db.execute(
        "SELECT id, username, email, company FROM users WHERE id = ?",
        (user_id,),
    )
    row = await cursor.fetchone()
    if not row:
